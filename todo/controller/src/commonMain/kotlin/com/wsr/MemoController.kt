...
class MemoController(private val productPath: (name: String) -> String) {
    ...
    ): MemoResponse = UpdateMemoUseCase(repository)
        .invoke(
            id = MemoId(id.value),
            title = title,
    ...
    suspend fun delete(id: MemoResponseId): MemoResponseId = DeleteMemoUseCase(repository)
        .invoke(id = MemoId(id.value))
    ...
    ): MemoResponse = UpdateItemUseCase(repository)
        .invoke(
    ...
    suspend fun moveItem(
        ...
        from: MemoResponseId,
        to: MemoResponseId,
    ): MemoResponse = MoveItemUseCase(repository)
        .invoke(
            itemId = ItemId(itemId.value),
